...
router.post('/login', function(req, res, next) {
  ...
  let query = {
    password: hashSHA256(password, 12),
    email
  }
  //console.log(query);
  ...
  knex('users')
    ...
    .then(user => {
      if (user.length === 0) {
        res.json({
          error: 'Login failed, please try again with a valid email and password, or Sign Up'
        });
      } else {
      ...
      if (user.length === 0) {
        ...
        knex('users')
          ...
          .then(newUser => {
            ...
            res.json({
              token
            }); //for es6 syntax your don't have to duplicate your object variable
          })
      } else {
        res.json({
